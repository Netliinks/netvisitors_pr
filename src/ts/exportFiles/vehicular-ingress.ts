...
export const exportVehiIngressIndPdf = async (ar: any) => {
    // @ts-ignore
    ...
    doc.text(80, 22, `INGRESO DE CONTENEDOR`);
    //Fin Cabecera
    let row = 35;
    //Cuerpo
    ...
    // Save the PDF
    var d = new Date();
    var title = "log_IngVehicularInd_" + d.getDate() + "_" + (d.getMonth() + 1) + "_" + d.getFullYear() + `.pdf`;
    doc.save(title);
};
export const exportVehiIngressPdf = async (ar: any) => {
    // @ts-ignore
    window.jsPDF = window.jspdf.jsPDF;
    // @ts-ignore
    var doc = new jsPDF();
    ...
    for (let i = 0; i < ar.length; i++) {
        ...
        doc.setFont(undefined, 'normal');
        doc.setTextColor(87, 80, 73);
        doc.text(42, row, `${vehicular.ingressDate} ${vehicular.ingressTime}`);
        doc.setFont(undefined, 'bold');
        doc.setTextColor(0, 0, 0);
        doc.setDrawColor(244, 244, 244);
        doc.setFillColor(244, 244, 244);
    ...
    // Save the PDF
    var d = new Date();
    var title = "log_IngVehicular_" + d.getDate() + "_" + (d.getMonth() + 1) + "_" + d.getFullYear() + `.pdf`;
    doc.save(title);
};
export const exportVehiIngressCsv = (ar: any, start: any, end: any) => {
    let rows = [];
    for (let i = 0; i < ar.length; i++) {
        let vehicular = ar[i];
        let obj = {
    ...
    generateFile(rows, "IngVehicular", "csv");
};
export const exportVehiIngressXls = (ar: any, start: any, end: any) => {
    let rows = [];
    for (let i = 0; i < ar.length; i++) {
        let vehicular = ar[i];
        let obj = {
    ...
    generateFile(rows, "IngVehicular", "xls");
};
