...
export const exportVehiSalIndPdf = async (ar: any) => {
    // @ts-ignore
...
export const exportVehiSalPdf = async (ar: any) => {
    // @ts-ignore
    window.jsPDF = window.jspdf.jsPDF;
    // @ts-ignore
    var doc = new jsPDF();
...
export const exportVehiSalCsv = (ar: any, start: any, end: any) => {
    let rows = [];
    for (let i = 0; i < ar.length; i++) {
        let vehicular = ar[i];
        let obj = {
            "Título": `${vehicular?.vehiMarcType ?? ''}`,
            "Fecha": `${vehicular.egressDate}`,
            "Hora": `${vehicular.egressTime}`,
            ...
            "Tipo": `${vehicular?.type ?? ''}`,
            "Usuario": `${vehicular?.egressIssued?.username ?? ''}`,
...
export const exportVehiSalXls = (ar: any, start: any, end: any) => {
    let rows = [];
    for (let i = 0; i < ar.length; i++) {
        let vehicular = ar[i];
        let obj = {
